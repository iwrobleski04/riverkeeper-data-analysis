...
def run():
    ...
    if page == "Basic Statistics":
        ...
        empty_rows = states[states.isna().all(axis=1)]
        st.write("Empty rows:", empty_rows)

        # Or rows that have empty index or first column
        st.write(states[states.iloc[:,0].astype(str).str.strip() == ''])
